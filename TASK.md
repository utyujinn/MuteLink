...
　【呼び出し】
　右コントローラーのスティック押し込み(`stick`)でキーボード表示のオン/オフを切り替える。単押し/長押しをそれぞれ独立してホットキー割り当てでき(`stick`/`stickLong`スロット)、既定は短押し=キーボード切り替え、長押し=送信取り消し。

　【レイアウト】
　4行5列グリッド。列1はモード切替(テンプレ/変換/数字・記号/英字)、列2〜4はかな入力(あかさたな…)、列5は編集操作(BS/空白/確定/送信)。下段にカーソル移動(◀/▶と5文字ジャンプの≪/≫)、その下に語尾ボタン(常設、設定 > 語尾で編集、テンプレ用の12個の定型文は設定 > テンプレートで編集)。
　フリックはコントローラーのポインター位置ベース(スティックではない): トリガーを引いた瞬間ポインターが指しているキーを「エンゲージ」し、トリガーを押している間のポインター位置で母音方向をライブに追従(センターに戻せばセンター文字に戻る、キー自身の枠を出たら即そちらの方向が確定)、離した瞬間の方向で入力される。押している間は隣のキー位置にフリック先の文字を重ねた十字を表示。
　押下時にコントローラーへ短い振動(`trigger_hand_haptic`)を返す。
　両手のレーザーポインター(コントローラー相対のドット+ビーム、常時表示)でボタンを狙って選択する。

　【かな漢字変換】
　「変換」ボタンでひらがな→漢字の変換候補を出す。ローカルで動く変換エンジン(Mozc等)があればそちらを優先、無ければGoogleの(非公式)Transliterate的なAPIを使う。
　→ ローカル変換は現実的でないと判断(調査した結果、Mozcは独自IPC+Protocol Buffers+Bazel/C++フルビルドが必要で、libkkc/AnthyはWindows/Rust側の実装が無い)。Googleの非公式transliterateエンドポイント(`ime.rs`の`convert_kana_to_kanji`)を実装。非公式・無保証なので、いつか死んだらここが直す場所。変換ボタンはカーソル直前の連続するひらがな部分を対象に変換候補を出し、もう一度押すと候補を巡回。「確定」ボタンで、他の操作をせずに変換モードだけ終える。

　【実装メモ】
　- フリック方向はスマホと同じ標準配置(タップ=あ段、左=い段、上=う段、右=え段、下=お段、`main.js`の`VR_KB_FLICK_ROWS`)。
