...
　右側にカーソル操作+履歴+クイック設定の2列5ブロック: 上段が1文字移動の◀/▶、下段が垂直移動(最上段・最下段で履歴へ移動)、さらにAuto送信/ホットキープロファイル切替、Chatbox/TTSのオンオフ(デスクトップの同名ボタンと状態を共有、オン中は控えめな濃紺でハイライト)。さらに下段に語尾ボタン(常設、設定 > 語尾で編集、テンプレ用の12個の定型文は設定 > テンプレートで編集)。履歴は語尾を除いた本文を最大200件保持する。
