...
interface SuffixConfig {
  ...
  speed: number; pitch: number; intonation: number; volume: number;
}
interface AppConfig {
  voicevox_path: string;
  concat_mode: boolean; concat_limit: number;
  auto_mode: boolean; tts_enabled: boolean; chatbox_enabled: boolean;
  tts_speaker_sel: number; tts_style_sel: number;
  tts_device_indices: boolean[];
  suffix_configs: SuffixConfig[] | null;
}
interface VoiceStyle { id: number; name: string; }
interface VoiceSpeaker { name: string; styles: VoiceStyle[]; }
interface AppState {
  is_running: boolean;
  rec_state: 'idle' | 'capturing' | 'speech_detected';
  hypothesis: string; pending: string | null;
  vrc_text: string; vrc_will_reset: boolean;
  auto_mode: boolean; concat_mode: boolean; concat_limit: number;
  chatbox_enabled: boolean; tts_enabled: boolean;
  tts_speaker_sel: number; tts_style_sel: number;
  tts_speakers: VoiceSpeaker[]; tts_devices: string[];
  tts_device_indices: boolean[];
  suffix_configs: SuffixConfig[];
  voicevox_path: string; last_error: string | null;
}
...
const errorLabel     = $<HTMLSpanElement>('error-label');
const hypothesisText = $<HTMLDivElement>('hypothesis-text');
const pendingText    = $<HTMLTextAreaElement>('pending-text');
const vrcArea        = $<HTMLDivElement>('vrc-area');
const vrcTextEl      = $<HTMLDivElement>('vrc-text');
const vrcCount       = $<HTMLSpanElement>('vrc-count');
const suffixBtns     = $<HTMLDivElement>('suffix-buttons');
...
const btnTts         = $<HTMLButtonElement>('btn-tts');
const settingsPanel  = $<HTMLDivElement>('settings-panel');
const suffixEditor   = $<HTMLDivElement>('suffix-editor');
const menuSettingsBtn = $<HTMLDivElement>('menu-settings-btn');
const menuSuffixBtn   = $<HTMLDivElement>('menu-suffix-btn');
const menuAppBtn      = $<HTMLDivElement>('menu-app-btn');
...
async function init() {
  ...
  // マイクデバイス一覧取得
  const devices: [string, string][] = await invoke('get_audio_devices');
  ...
  // Tauri イベント受信
  await listen<AppState>('app-state', (e) => updateUI(e.payload));
  ...
  vrcArea.addEventListener('click', () => {
    if (st?.vrc_text) invoke('reset_chatbox');
  });
  ...
  // 言語・マイク変更 → 認識再起動
  langSelect.addEventListener('change', () => { if (st?.is_running) restartRecognition(); });
  micSelect.addEventListener('change',  () => { if (st?.is_running) restartRecognition(); });
  ...
  // 初期状態を取得（起動時に app-state イベントが来ないため）
  const initialState: AppState = await invoke('get_state');
  updateUI(initialState);
}

// ── UI 更新 ──────────────────────────────────────────────────────────
function updateUI(s: AppState) {
  st = s;
  ...
  suffixBtns.querySelectorAll<HTMLButtonElement>('.suffix-btn').forEach(b => {
...
function buildSuffixButtons(configs: SuffixConfig[]) {
...
function toggleBoolSetting(key: 'auto_mode' | 'chatbox_enabled' | 'tts_enabled') {
  if (!st) return;
...
function confirmWithSuffix(suffixIdx: number) {
...
function stateToConfig(s: AppState): AppConfig {
  return {
...
function startSpeech(lang: string) {
  ...
  r.onend = () => { if (st?.is_running) setTimeout(() => r.start(), 200); };
  r.start();
  recognition = r;
}
...
function buildSettingsPanelHTML() {
  ...
  $<HTMLButtonElement>('settings-close').addEventListener('click', toggleSettingsPanel);

  ($<HTMLInputElement>('s-concat')).addEventListener('change', (e) => {
    if (!st) return;
    const cfg = stateToConfig(st);
    cfg.concat_mode = (e.target as HTMLInputElement).checked;
    invoke('update_settings', { cfg });
  });
  ($<HTMLInputElement>('s-concat-limit')).addEventListener('change', (e) => {
    if (!st) return;
    const cfg = stateToConfig(st);
    cfg.concat_limit = Math.max(1, parseInt((e.target as HTMLInputElement).value, 10) || 50);
    invoke('update_settings', { cfg });
  });
  ($<HTMLInputElement>('s-chatbox')).addEventListener('change', (e) => {
    if (!st) return;
    const cfg = stateToConfig(st);
    cfg.chatbox_enabled = (e.target as HTMLInputElement).checked;
    invoke('update_settings', { cfg });
  });
  ($<HTMLInputElement>('s-tts')).addEventListener('change', (e) => {
    if (!st) return;
    const cfg = stateToConfig(st);
    cfg.tts_enabled = (e.target as HTMLInputElement).checked;
    invoke('update_settings', { cfg });
  });

  // スピーカー変更 → スタイルセレクタを即時更新してから保存
  $<HTMLSelectElement>('s-speaker').addEventListener('change', () => {
    if (!st) return;
    const spkIdx = parseInt(($<HTMLSelectElement>('s-speaker')).value, 10);
    const spk = st.tts_speakers[spkIdx];
    if (spk) {
      const styleEl = $<HTMLSelectElement>('s-style');
      styleEl.innerHTML = '';
  ...
  $<HTMLSelectElement>('s-style').addEventListener('change', () => {
    if (!st) return;
    const cfg = stateToConfig(st);
    cfg.tts_style_sel = parseInt(($<HTMLSelectElement>('s-style')).value, 10);
    invoke('update_settings', { cfg });
  });

  $<HTMLButtonElement>('s-vv-detect').addEventListener('click', async () => {
    const path: string = await invoke('find_voicevox_path');
    $<HTMLInputElement>('s-vv-path').value = path;
    setVvStatus(path);
    if (!st) return;
    const cfg = stateToConfig(st);
  ...
  $<HTMLInputElement>('s-vv-path').addEventListener('change', (e) => {
...
function setVvStatus(path: string) {
  const el = $<HTMLSpanElement>('s-vv-status');
  if (!path) {
    el.textContent = '未設定';
...
function refreshSettingsPanel(s: AppState) {
  ($<HTMLInputElement>('s-concat')).checked       = s.concat_mode;
  ($<HTMLInputElement>('s-concat-limit')).value   = String(s.concat_limit);
  ($<HTMLInputElement>('s-chatbox')).checked      = s.chatbox_enabled;
  ($<HTMLInputElement>('s-tts')).checked          = s.tts_enabled;

  // スピーカーセレクタ（リストが変わったとき再構築）
  const spkEl   = $<HTMLSelectElement>('s-speaker');
  const styleEl = $<HTMLSelectElement>('s-style');
  ...
  if (spkEl.value !== String(spkIdx)) spkEl.value = String(spkIdx);
  ...
  if (styleEl.value !== String(styleIdx)) styleEl.value = String(styleIdx);

  // 出力デバイス（デバイス数が変わったとき再構築）
  const devContainer = $<HTMLDivElement>('s-tts-devices');
  if (s.tts_devices.length !== lastDeviceCount) {
    ...
    devContainer.querySelectorAll<HTMLInputElement>('input[data-idx]').forEach(cb => {
  ...
  const pathEl = $<HTMLInputElement>('s-vv-path');
...
function buildSuffixEditorHTML() {
  ...
  `;
  $<HTMLButtonElement>('suffix-close').addEventListener('click', toggleSuffixEditor);
}

function refreshSuffixEditor(configs: SuffixConfig[]) {
  const tbody = $<HTMLTableSectionElement>('suffix-tbody');
  tbody.innerHTML = '';
  configs.forEach((cfg, i) => {
    ...
    `;
    tr.querySelectorAll<HTMLInputElement>('.sf-i').forEach(inp => {
      inp.addEventListener('change', () => saveSuffixRow(tr, i));
    });
    tr.querySelector<HTMLButtonElement>('.sf-reset-btn')!.addEventListener('click', () => {
      resetSuffixRow(tr, i);
    });
...
function saveSuffixRow(tr: HTMLTableRowElement, i: number) {
  if (!st) return;
  const cfg = stateToConfig(st);
  if (!cfg.suffix_configs) return;
  cfg.suffix_configs[i] = {
    label:      (tr.querySelector<HTMLInputElement>('.sf-label')! ).value,
    suffix:     (tr.querySelector<HTMLInputElement>('.sf-suffix')!).value,
    speed:      parseFloat((tr.querySelector<HTMLInputElement>('.sf-speed')!).value) || 1.0,
    pitch:      parseFloat((tr.querySelector<HTMLInputElement>('.sf-pitch')!).value) || 0.0,
    intonation: parseFloat((tr.querySelector<HTMLInputElement>('.sf-inton')!).value) || 1.0,
    volume:     parseFloat((tr.querySelector<HTMLInputElement>('.sf-vol')!  ).value) || 1.0,
  };
  invoke('update_settings', { cfg });
}

function resetSuffixRow(tr: HTMLTableRowElement, i: number) {
  const def = DEFAULT_SUFFIXES[i] ?? DEFAULT_SUFFIXES[0];
  (tr.querySelector<HTMLInputElement>('.sf-label')! ).value = def.label;
  (tr.querySelector<HTMLInputElement>('.sf-suffix')!).value = def.suffix;
  (tr.querySelector<HTMLInputElement>('.sf-speed')! ).value = def.speed.toFixed(2);
  (tr.querySelector<HTMLInputElement>('.sf-pitch')! ).value = def.pitch.toFixed(3);
  (tr.querySelector<HTMLInputElement>('.sf-inton')! ).value = def.intonation.toFixed(2);
  (tr.querySelector<HTMLInputElement>('.sf-vol')!   ).value = def.volume.toFixed(2);
  saveSuffixRow(tr, i);
}
...
function toggleSuffixEditor() {
  ...
  if (!wasHidden) {
    ...
    if (st) refreshSuffixEditor(st.suffix_configs);
  }
}
...
function toggleAppDropdown() {
  ...
  $<HTMLDivElement>('dd-reset').addEventListener('click', async () => {
    if (!st) return;
    const cfg: AppConfig = {
  ...
  $<HTMLDivElement>('dd-quit').addEventListener('click', async () => {
