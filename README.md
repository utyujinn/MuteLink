...
- Rustバックエンド(`tauri-src/src-tauri/`)
  ...
  - `overlay.rs`/`overlay_gpu.rs`: SteamVR上に表示する確認/破棄HUD・VRキーボード・レーザーポインターをフォント手動ラスタライズ+`SetOverlayTexture`で描画。`lib.rs`にVR内フリック入力キーボード(コントローラーのポインターでキーを狙い、トリガーを引いたままスティック/ポインター位置で母音方向をフリック。表示位置は視界追従(HMD相対)か、開いた位置に固定(絶対座標、グリップでつかんで移動可)かを設定で選択。キーボード表示中は確認/破棄HUDの文字列をポインターで狙ってトリガーを引くとその位置にカーソルを移動)とかな漢字変換(`ime.rs`、Googleの非公式transliterateエンドポイント経由)を実装
